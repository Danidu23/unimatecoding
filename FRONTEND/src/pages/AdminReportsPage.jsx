...
import api from '../api';
import { FiDownload, FiFilter, FiStar } from 'react-icons/fi';
...
const AdminReportsPage = () => {
  ...
  const [feedbackData, setFeedbackData] = useState({ recentFeedback: [], averages: [] });

  useEffect(() => {
    const fetchReports = async () => {
      try {
        const [dashRes, bookRes, facRes, feedRes] = await Promise.all([
        ...
        setFeedbackData(feedRes.data || { recentFeedback: [], averages: [] });
      } catch (err) {
    ...
    <div className="admin-reports-page">
      ...
      <div className="reports-grid">
        ...
        <div className="report-section">
          ...
          <div className="facility-usage-report">
            {feedbackData.averages.map((f) => (
              <div key={f.name} className="fur-row" style={{ alignItems: 'center' }}>
                <div className="fur-name">{f.name}</div>
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <FiStar color="var(--accent-primary)" fill="var(--accent-primary)" />
                  <strong style={{ fontSize: '1.1rem' }}>{f.avgRating}</strong>
                  <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>({f.totalRatings} reviews)</span>
                </div>
              </div>
            ))}
            {feedbackData.averages.length === 0 && <p style={{color: 'var(--text-muted)'}}>No rating data</p>}
          </div>
        </div>
      </div>
      ...
      <div className="report-section" style={{ marginTop: '24px' }}>
        ...
        <div className="table-container" style={{ marginTop: 16 }}>
          <table className="table">
            ...
            <tbody>
              {feedbackData.recentFeedback.slice(0, 10).map(b => (
                <tr key={b._id}>
                  <td style={{ fontSize: '0.82rem' }}>{b.date}</td>
                  <td style={{ fontWeight: 600, fontSize: '0.875rem' }}>{b.facilityServiceId?.name}</td>
                  <td>{b.userId?.name}</td>
                  <td style={{ color: 'var(--accent-primary)', fontWeight: 'bold' }}>★ {b.rating}/5</td>
                  <td style={{ maxWidth: '300px', whiteSpace: 'normal', fontSize: '0.9rem' }}>{b.feedback || <span style={{color: 'var(--text-muted)'}}>No comment</span>}</td>
                </tr>
              ))}
              {feedbackData.recentFeedback.length === 0 && (
                <tr>
                  <td colSpan="5" style={{textAlign: 'center', color: 'var(--text-muted)'}}>No feedback received yet</td>
                </tr>
              )}
            </tbody>
          </table>
