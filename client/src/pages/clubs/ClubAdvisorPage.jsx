...
import { ArrowLeft, Bot, Check, Loader, Sparkles } from "lucide-react";
        ...
        <div style={{ padding: "18px 22px 22px" }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: "14px" }}>
              ...
              <textarea className="form-input" rows={3} placeholder="Why do you want to join? (min 50 chars)" value={form.reason} onChange={e => setForm(p => ({ ...p, reason: e.target.value }))} style={{ resize: "vertical" }} disabled={isDrafting || loading} />
            </div>
  ...
  const handleNext = async () => {
    if (step < 2) {
      ...
      try {
        ...
        localStorage.setItem("clubsQuiz", JSON.stringify({
          skills: answers.skills,
          activities: answers.activities,
        ...
        <div className="quiz-card">
          {!done ? (
            ...
            <div style={{ animation: "fadeIn .5s ease" }}>
              ...
              {explanation && (
                <div style={{ background: "rgba(255,255,255,.03)", border: "1.5px solid rgba(255,215,0,.15)", borderRadius: "16px", padding: "18px 20px", marginBottom: "24px", color: "rgba(255,255,255,.75)", lineHeight: 1.7, fontSize: "14px" }}>
                  {explanation}
                </div>
              )}

              {recommendations.length === 0 ? (
                <div style={{ background: "rgba(255,255,255,.03)", border: "1.5px solid rgba(255,215,0,.15)", borderRadius: "16px", padding: "24px", color: "rgba(255,255,255,.7)", textAlign: "center" }}>
                  No strong matches found yet. Try retaking the quiz or browse all clubs.
                </div>
              ) : (
              ...
              <div style={{ display: "flex", gap: "12px", marginTop: "32px" }}>
                <button onClick={() => { setDone(false); setStep(0); setAnswers({ skills: [], activities: [], commitment: "" }); setExplanation(""); }} style={{ flex: 1, background: "rgba(255,255,255,.05)", padding: "14px", borderRadius: "12px", color: "#fff", fontSize: "14px", fontWeight: 700 }}>Retake</button>
