...
import {
  Users, ClipboardList, BookOpen, LogOut,
...
const CSS = `
  ...
  *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
  ...
  .status-badge.Interview{background:rgba(96,165,250,.1);border:1px solid rgba(96,165,250,.2);color:#60a5fa;}
...
function ClubModal({ club, onClose, onSave }) {
  const [form, setForm] = useState(
    club
      ? {
          ...
          gallery: Array.isArray(club.gallery) ? club.gallery.join("\n") : club.gallery || "",
        }
      : {
          name: "",
          category: "Technology",
  ...
  const handleSubmit = async (e) => {
    ...
    if (!about || about.length < 40) return setError("About section must be at least 40 characters.");
    ...
    <div className="modal-bg" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="modal-body">
          ...
          {[
            { label: "Club Name", key: "name", ph: "e.g. CodeSpace Club" }, 
            { label: "Brand Logo URL (optional)", key: "logoUrl", ph: "https://..." }
           ].map(f => (
          ...
          <div className="form-field" style={{ marginBottom: "16px" }}>
            ...
            <textarea
              rows={5}
              placeholder="Write the full about section shown on the club details view..."
              value={form.about || ""}
          ...
          <div className="form-field" style={{ marginBottom: "24px" }}>
            ...
            <textarea
              rows={4}
              placeholder={"https://...\nhttps://..."}
              value={typeof form.gallery === "string" ? form.gallery : form.gallery?.join("\n")}
...
function InterviewModal({ application, onConfirm, onCancel }) {
  ...
  return (
    <div className="modal-bg" onClick={onCancel}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          <h3 style={{ fontSize: "18px", fontWeight: 900, color: "#fff" }}>Schedule Interview</h3>
          <button onClick={onCancel} style={{ background: "rgba(255,255,255,.05)", border: "none", color: "rgba(255,255,255,0.6)", width: "36px", height: "36px", borderRadius: "10px", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: "all 0.2s" }} onMouseEnter={e => e.currentTarget.style.color="#fff"} onMouseLeave={e => e.currentTarget.style.color="rgba(255,255,255,0.6)"}><X size={18} /></button>
        </div>
        <div className="modal-body">
          <div style={{ marginBottom: "16px", color: "rgba(255,255,255,.6)", fontSize: "13px" }}>
            Scheduling interview for <span style={{ color: "#fff", fontWeight: 800 }}>{application.studentName}</span>
          </div>
          ...
          <div style={{ display: "flex", gap: "12px" }}>
            <button className="ad-btn sec" style={{ flex: 1, padding: "12px" }} onClick={onCancel}>Cancel</button>
            <button className="ad-btn" style={{ flex: 1, padding: "12px" }} onClick={() => onConfirm(interviewAt)}>Confirm</button>
          </div>
        </div>
      </div>
    </div>
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const hasClubsAdminAccess =
      currentUser?.role === "admin" &&
      currentUser?.permissions?.includes("clubs_admin");

    if (!currentSession) {
      navigate("/login");
      return;
    }
    if (!hasClubsAdminAccess) {
      navigate("/clubs");
      return;
    }
    loadAll();
  ...
  const handleInterviewSchedule = async (id, interviewAt) => {
    try {
      await adminAPI.updateApplication(id, { status: "Interview", interviewAt });
      showToast("Interview scheduled successfully!");
      const res = await adminAPI.getApplications(appFilter);
      setApplications(res.data);
      const s = await adminAPI.getStats(); setStats(s.data);
    } catch (err) {
      showToast("Interview scheduling failed. Please try again.");
    }
  };
    ...
    <>
      ...
      {interviewModal && (
        <InterviewModal
          ...
          onConfirm={(interviewAt) => { setInterviewModal(null); handleInterviewSchedule(interviewModal._id, interviewAt); }}
      ...
      <div className="adm-layout">
        <aside className="adm-sidebar">
          ...
          <div style={{ padding: "20px 16px", borderTop: "1px solid rgba(255,255,255,.06)" }}>
            <button className="nav-item" onClick={() => { clearSession(); navigate("/login"); }} style={{ color: "#f87171" }}>
        ...
        <main className="adm-main">
          <header className="adm-header">
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              <button className="ad-btn sec" onClick={handleSync}>
                <RefreshCw size={14} className={syncing ? "animate-spin" : ""} /> Sync Data
              </button>
              {activeTab === "clubs" && (
                            ...
                            <td>
                              <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                                ...
                                {a.status === "Interview" && a.interviewAt && (
                                  <span style={{ fontSize: "11px", color: "rgba(255,255,255,.45)", fontWeight: 600 }}>
                                    {new Date(a.interviewAt).toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}
                                  </span>
                                )}
                              </div>
                            </td>
                        ...
                        {filteredApps.map(a => (
                          <tr key={a._id} style={{ opacity: a.status === "Pending" ? 1 : 0.7 }}>
                            ...
                            <td>
                              <div style={{ display: "flex", gap: "8px" }}>
                                {a.status === "Pending" && (
                                  <button className="ad-btn sec" style={{ padding: "8px 12px" }} onClick={() => setInterviewModal(a)}>Interview</button>
                                )}
                                {a.status !== "Approved" && (
