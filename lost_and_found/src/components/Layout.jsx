...
import { MOCK_NOTIFICATIONS } from '../data/lostFoundAdvanced';
      ...
      <nav className={`c-nav${scrolled ? " scrolled" : ""}`}>
        ...
        <div className="desktop-nav" style={{ display:"flex", gap:"clamp(18px,3vw,32px)" }}>
          {["Dashboard","Canteen","Lost & Found","Messages","Sports","Clubs","Orders"].map((item,i) => {
             ...
             return (
              <a key={i} href={path} className={`nav-lnk${location.pathname === path || (item === 'Lost & Found' && location.pathname === '/') ? " active" : ""}`}
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"4px", position: "relative" }}>
          ...
          {showNotifications ? <NotificationDropdown items={MOCK_NOTIFICATIONS} onClose={() => setShowNotifications(false)} /> : null}
