...
export default function BrowseItems() {
  ...
  return (
    <div style={{ width: '100%', background: '#07091a', minHeight: 'calc(100vh - 66px)' }}>
      ...
      <div style={{ position: 'sticky', top: '66px', zIndex: 100, background: 'rgba(7,9,26,.98)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,.07)', padding: '16px clamp(20px,6vw,60px) 14px' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '14px', alignItems: 'center', justifyContent: 'space-between' }}>
            <div className='search-wrap' style={{ flex: '1 1 320px', maxWidth: '560px', margin: 0, background: 'rgba(255,255,255,.05)' }}>
            ...
            <button className='btn-outline' style={{ padding: '10px 14px', borderRadius: '12px', fontSize: '13px' }} onClick={() => { setSearch(''); setActiveCategory('All'); setActiveType('all'); setLocationZone('ALL'); }}>
              Reset All
            </button>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px' }}>
            <div style={{ background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.08)', borderRadius: '16px', padding: '12px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                {['all', 'lost', 'found'].map(t => (
                  <button key={t} onClick={() => setActiveType(t)} style={{ flex: 1, padding: '10px 12px', borderRadius: '10px', fontSize: '13px', fontWeight: 700, fontFamily: 'Manrope,sans-serif', textTransform: 'capitalize', background: activeType === t ? (t === 'lost' ? 'rgba(239,68,68,.16)' : t === 'found' ? 'rgba(34,197,94,.16)' : 'rgba(245,166,35,.16)') : 'rgba(255,255,255,.04)', color: activeType === t ? (t === 'lost' ? '#f87171' : t === 'found' ? '#34d399' : '#F5A623') : 'rgba(255,255,255,.58)', border: `1px solid ${activeType === t ? (t === 'lost' ? 'rgba(239,68,68,.3)' : t === 'found' ? 'rgba(34,197,94,.3)' : 'rgba(245,166,35,.3)') : 'rgba(255,255,255,.08)'}`, transition: 'all .2s' }}>
                    {t}
                  </button>
                ))}
              </div>
            ...
            <div style={{ background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.08)', borderRadius: '16px', padding: '12px' }}>
              ...
              <select className='form-select' value={locationZone} onChange={(e) => setLocationZone(e.target.value)} style={{ margin: 0, padding: '10px 12px', borderRadius: '10px' }}>
