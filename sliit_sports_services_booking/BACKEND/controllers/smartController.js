...
const getSmartSuggestions = async (req, res) => {
    try {
        ...
        const slots = await Slot.find({ facilityServiceId, date, status: { $ne: 'blocked' } });
...
const getOccupancyInfo = async (req, res) => {
    try {
        ...
        const slots = await Slot.find({ facilityServiceId, date });
...
const getPriorityStats = async (req, res) => {
    try {
        const priorityBookings = await Booking.find({ isPriority: true })
